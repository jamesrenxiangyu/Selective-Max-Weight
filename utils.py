...
def discrete_action_user_coupled(action):
    action_discrete = np.zeros(40)
    for i in range(20):
        action_discrete[i] = action_value_user(action[i])
        action_discrete[i+20] = action_value_user(action[i])
    return action_discrete


def discrete_action_user(action):
    action_discrete = np.zeros(20)
    for i in range(20):
        action_discrete[i] = action_value_user(action[i])
    return action_discrete
...
def action_value_cache(a):
    if a < 1.2 and a > -1.2:
        return math.floor((a+1.2)/0.08)
    return random.randrange(0, 30)

def discrete_action(action):
    action1 = np.zeros(8)
    action1[0] = action_value_speed(action[0])
    action1[1] = action_value_direction(action[1])
    action1[2] = action_value_power(action[2])
    action1[3] = action_value_cache(action[3])
    action1[4] = action_value_cache(action[4])
    action1[5] = action_value_cache(action[5])
    action1[6] = action_value_cache(action[6])
    action1[7] = action_value_cache(action[7])
    return action1
